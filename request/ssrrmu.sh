...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} La cuenta actual no es ROOT (no tiene permiso ROOT), no puede continuar la operaci�n, por favor ${Green_background_prefix} sudo su ${Font_color_suffix} Venga a ROOT (le pedir� que ingrese la contrase�a de la cuenta actual despu�s de la ejecuci�n)" && exit 1
}
check_sys(){
...
echo -e "$BARRA1"
 read -p "(Predeterminado: cancelar):" View_user_port
...
#Configuraci�n de la informaci�n de configuraci�n
Set_config_user(){
echo -e "$BARRA1"
...
echo -e "$BARRA"
read -p "(Predeterminado: Tesla):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="Tesla"
...
echo -e "$BARRA"
	 read -p "(Predeterminado: 445):" ssr_port
...
echo -e "$BARRA"
	 read -p "(Predeterminado: Tesla):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="Tesla"
...
echo -e "$BARRA1"
	 read -p "(Predeterminado: 10. aes-256-cfb):" ssr_method
...
echo -e "$BARRA1"
	...
	if [[ ${ssr_obfs} != "plain" ]]; then
			 read -p "Configurar el complemento de protocolo en modo compatible(_compatible)?[Y/n]" ssr_obfs_yn
...
Set_user_api_server_pub_addr(){
	...
	echo "${ama_font_prefix}Introduzca la IP del servidor o el nombre de dominio que se mostrara en la configuraci�n del usuario${Font_color_suffix}"
echo -e "$BARRA1"
	 read -p "(Predeterminado: Predeterminado:Deteccion automatica de la red externa IP):" ssr_server_pub_addr
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		wget  --no-check-certificate -N "https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_url}/libsodium-${Libsodiumr_ver}.tar.gz"
	...
	echo && echo -e "${Info} libsodium �xito de instalaci�n!" && echo
${SCPfrm}/ssrrmu.sh
}
...
echo -e "$BARRA1"
		 read -p "(Predeterminado: cancelar):" ssr_port
...
${BARRA}
 ${Green_font_prefix}[2] >${Font_color_suffix}  ${blan_font_prefix}Eliminar la configuraci�n del usuario${Font_color_suffix}
${BARRA}
	...
	if [[ "$lalal" == "install" ]]; then
		...
		do
			...
			if [[ -z "${match_add}" ]]; then
				...
				echo
				 read -p "Continuar para agregar la configuracion del usuario?[Y/n]:" addyn
				[[ -z ${addyn} ]] && addyn="y"
				if [[ ${addyn} == [Nn] ]]; then
					Get_User_info "${ssr_port}"
					View_User_info
					...
					echo -e "${Info} Continuar agregando configuraci�n de usuario ..."
				fi
			fi
		done
	fi
...
echo -e "$BARRA"
		 read -p "(Predeterminado:Cancelar):" del_user_port
...
echo -e "$BARRA"
		 read -p "(Predeterminado: Cancelar):" Clear_transfer_user_port
...
${BARRA}
 ${Green_font_prefix} 0 2 */7 * * ${Font_color_suffix} Representante 7 d�as 2: 0 minutos despeja el trafico usado.�
${BARRA}
...
echo -e "$BARRA1"
	 read -p "(Predeterminado: Cancelar):" server_speeder_num
...
echo -e "$BARRA1"
	 ...
	 read -p "(Predeterminado: Cancelar):" lotserver_num
...
Install_LotServer(){
	[[ -e ${LotServer_file} ]] && echo -e "${Error} LotServer est� instalado!" && exit 1
...
echo -e "$BARRA1"
	 read -p "(Predeterminado: Cancelar):" bbr_num
echo -e "$BARRA1"
...
echo -e "$BARRA1"
	 read -p "(Predeterminado: cancelar):" other_num
...
echo -e "$BARRA"
		echo -e "[yes] ${blan_font_prefix}para cambiar a ${Green_font_prefix}Modo detallado (registro de conexion + registro de errores)${Font_color_suffix}？[y/N]"
echo -e "$BARRA"
...
crontab_monitor_ssr(){
	...
	if [[ -z ${PID} ]]; then
		...
		check_pid
		if [[ -z ${PID} ]]; then
			echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] Fall� el inicio del servidor ShadowsocksR..." | tee -a ${ssr_log_file} && exit 1
		else
...
crontab_monitor_ssr_cron_start(){
	...
	if [[ -z ${cron_config} ]]; then
		echo -e "${Error} Fall� el arranque del servidor ShadowsocksR!" && exit 1
	else
...
crontab_monitor_ssr_cron_stop(){
	...
	cron_config=$(crontab -l | grep "ssrmu.sh monitor")
	if [[ ! -z ${cron_config} ]]; then
		echo -e "${Error} Fall� la detencion del servidor ShadowsocksR!" && exit 1
	else
...
clear
	echo -e "  ${blue_font_prefix}ShadowsocksR Manager LA CASITA DEL TERROR${Font_color_suffix}  ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
${BARRA1}
...
${Green_font_prefix}  [16] >${Font_color_suffix} ${blan_font_prefix}SALIR${Font_color_suffix}
 "
	menu_status
	echo &&  read -p "Porfavor seleccione una opcion [1-16]:" num
case "$num" in
	1)Install_SSR ;;
	2)Update_SSR ;;
	3)Uninstall_SSR ;;
